# US-1706 — Create `crux-plugin-terraform-aws` Plugin
...
As a user of Crux,
I want the `crux-plugin-terraform-aws` plugin available and installable,
so that I can generate a service with Terraform AWS (RDS, ElastiCache, MSK, IAM) integration pre-configured and production-ready.
...
Implement the `crux-plugin-terraform-aws` plugin including its `plugin.yaml` manifest, prompt questions, templates, and lifecycle hooks.

### Plugin Deliverables

- `plugin.yaml` manifest with metadata, version, trust tier, questions, and template references
- All templates required to integrate Terraform AWS (RDS, ElastiCache, MSK, IAM) into the generated service
...
- [ ] Integration tested against a real Terraform AWS (RDS, ElastiCache, MSK, IAM) instance in the local environment
