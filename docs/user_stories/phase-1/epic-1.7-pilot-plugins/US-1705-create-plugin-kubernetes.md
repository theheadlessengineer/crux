# US-1705 — Create `crux-plugin-kubernetes` Plugin
...
As a user of Crux,
I want the `crux-plugin-kubernetes` plugin available and installable,
so that I can generate a service with Kubernetes (Deployment, Service, HPA, PDB, Network Policy) integration pre-configured and production-ready.
...
Implement the `crux-plugin-kubernetes` plugin including its `plugin.yaml` manifest, prompt questions, templates, and lifecycle hooks.

### Plugin Deliverables

- `plugin.yaml` manifest with metadata, version, trust tier, questions, and template references
- All templates required to integrate Kubernetes (Deployment, Service, HPA, PDB, Network Policy) into the generated service
...
- [ ] Integration tested against a real Kubernetes (Deployment, Service, HPA, PDB, Network Policy) instance in the local environment
