# US-1701 — Create `crux-plugin-postgresql` Plugin
...
As a user of Crux,
I want the `crux-plugin-postgresql` plugin available and installable,
so that I can generate a service with PostgreSQL (Pool, Migrations, Read Replica, Audit Logging) integration pre-configured and production-ready.
...
Implement the `crux-plugin-postgresql` plugin including its `plugin.yaml` manifest, prompt questions, templates, and lifecycle hooks.

### Plugin Deliverables

- `plugin.yaml` manifest with metadata, version, trust tier, questions, and template references
- All templates required to integrate PostgreSQL (Pool, Migrations, Read Replica, Audit Logging) into the generated service
...
- [ ] Integration tested against a real PostgreSQL (Pool, Migrations, Read Replica, Audit Logging) instance in the local environment
