# US-1708 — Create `crux-plugin-prometheus` Plugin
...
As a user of Crux,
I want the `crux-plugin-prometheus` plugin available and installable,
so that I can generate a service with Prometheus (Metrics, Alerts, Dashboard) integration pre-configured and production-ready.
...
Implement the `crux-plugin-prometheus` plugin including its `plugin.yaml` manifest, prompt questions, templates, and lifecycle hooks.

### Plugin Deliverables

- `plugin.yaml` manifest with metadata, version, trust tier, questions, and template references
- All templates required to integrate Prometheus (Metrics, Alerts, Dashboard) into the generated service
...
- [ ] Integration tested against a real Prometheus (Metrics, Alerts, Dashboard) instance in the local environment
